...
> **核心设计**：前端只认 `:8088`，只写 `/api/*` 相对路径。后端是 1 个单体还是 5 个微服务，前端代码零改动。

| 后端部署方式 | 前端改动 | 说明 |
|-------------|---------|------|
| 单体 `rag-server :8080` | 无 | Gateway `/api/**` → `rag-server:8080` |
| 微服务 `rag-auth :8081` 等 5 个 | 无 | Gateway 按路径分发到不同服务 |
| 扩缩容、加机器、换服务 | 无 | 改 Gateway 路由表即可 |
